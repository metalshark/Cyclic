...
public class TileCableItem extends TileEntityBase implements ITickableTileEntity, INamedContainerProvider {
  ...
  public void tick() {
    ...
    for (@Nonnull final Direction side : UtilDirection.getAllInDifferentOrder()) {
      ...
      if (connectType == EnumConnectType.CABLE) {
        remainingAmount -= getItemsFromAdjacent(itemHandler, side, remainingAmount);
  ...
  public void invalidateCaps() {
    ...
    itemCap.invalidate();
    for (@Nonnull final LazyOptional<IItemHandler> sidedCap : itemCapSides.values()) {
      sidedCap.invalidate();
    }
  }
  ...
  @Nullable
  @Override
  public Container createMenu(final int i, @Nonnull final PlayerInventory playerInventory, @Nonnull final PlayerEntity playerEntity) {
    if (world == null) {
      return null;
    }
