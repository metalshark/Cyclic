...
public class TileGeneratorFuel extends TileEntityBase implements INamedContainerProvider, ITickableTileEntity {
  ...
  final int factor = 1;
  CustomEnergyStorage energyStorage = new CustomEnergyStorage(MAX, MAX) {
    @Override
  ...
  ItemStackHandler inputSlots = new ItemStackHandler(1) {
    @Override
    public boolean isItemValid(int slot, @Nonnull final ItemStack itemStack) {
      return ForgeHooks.getBurnTime(itemStack, IRecipeType.SMELTING) > 0; //stack.getBurnTime(IRecipeType.SMELTING) >= 0;
    }
  };
  ...
  public void tick() {
    ...
    if (this.burnTime <= 0) {
      setLitProperty(false);
    } else if (currentEnergy < energyStorage.getMaxEnergyStored()) {
      setLitProperty(true);
      this.burnTime--;
